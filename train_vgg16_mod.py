...
def cnn_train(batch, x_train, y_train, x_test, y_test):
    ...
    with tf.Session() as sess:
        ...
        last_epoch = load_model(sess, saver, '/home/chenchen/PycharmProjects/tensorflow_cifar/save_model_vgg16_mod/')

        # sess.run(tf.global_variables_initializer())
        summary_writer = tf.summary.FileWriter('./tmp_vgg16_mod', graph=tf.get_default_graph())
        ...
        for n in range(last_epoch + 1, 1000):
                ...
                if (n*num_batch+i) % 50 == 0:
                    ...
                    if acc > 0.77:
                        constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                                   ["outnode"])
                        # 保存训练出来的模型文件
                        with tf.gfile.FastGFile("/home/chenchen/PycharmProjects/tensorflow_cifar/android_vgg16_mod/vgg16_mod_model-"+str(acc)+".pb", mode='wb') as f:
            ...
            # 每次训练一遍保存一次模型，以便中断时可以从上次恢复
            saver.save(sess, '/home/chenchen/PycharmProjects/tensorflow_cifar/save_model_vgg16_mod/cifar.model', global_step=n)
        print('准确度小于0.81!')

if __name__ == '__main__':

    cifar10_dir = '/home/chenchen/PycharmProjects/tensorflow_cifar/cifar-10-batches-py/'
